  ...
  return (
    <div className="max-w-3xl mx-auto px-6 sm:px-8 py-16 md:py-24">
      ...
      <div className="space-y-8 welcome-fade" style={{ animationDelay: '0.1s' }}>
        {posts.length === 0 && (
          <p className="text-gray-400 dark:text-gray-500 italic">No posts yet.</p>
        )}
        {posts.map((post) => (
          <Link
            key={post.slug}
            to={`/blog/${post.slug}`}
            className="group block pb-8 border-b border-gray-100 dark:border-gray-800 last:border-0"
          >
            <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100 group-hover:text-teal-700 dark:group-hover:text-teal-400 transition-colors leading-snug">
              {post.title}
            </h2>
